...
class OperationTest extends GPWebPayTestCase
{
	...
	{
		Assert::exception(function () use ($orderNum) {
			new Operation($orderNum, 1000, 203);
		}, InvalidArgumentException::class, 'ORDERNUMBER must by type of numeric ' . gettype($orderNum) . ' given');
	}
	...
	public function testCreateOperationException()
	{
		...
		$amount = '1000';
		Assert::exception(function () use ($amount) {
			new Operation(123456, $amount, 203);
		}, InvalidArgumentException::class, 'AMOUNT must by type of INT or FLOAT !' . gettype($amount) . ' given');

		$currency = 'CZK';
		Assert::exception(function () use ($currency) {
			new Operation(123456, 1000, $currency);
		}, InvalidArgumentException::class, 'CURRENCY must by INT ! ' . gettype($currency) . ' given');
